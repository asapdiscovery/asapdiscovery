...
def main():
    ...
    # barrier: could make parallel here as well possibly as do not rely on collected results
    ligand_selector: LigandSelector = PairwiseLigandSelector(prepped_ligands, prepped_targets)
    docking_inputs: List[DockingInput] = ligand_selector.select()

    docking_results = []
    for docking_input in docking_inputs:
        dr = docking_wf(docking_input)
