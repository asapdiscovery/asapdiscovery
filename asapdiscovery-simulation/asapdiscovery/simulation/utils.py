...
from asapdiscovery.simulation.schema.fec import AlchemiscaleResults, AlchemiscaleSettings, FreeEnergyCalculationNetwork
...
class AlchemiscaleHelper:
    ...
    def __init__(self):
        ...
        self._client = AlchemiscaleClient(api_url='https://api.alchemiscale.org', identifier=settings.ALCHEMISCALE_ID, key=settings.ALCHEMISCALE_KEY)

    def create_network(self, planned_network: FreeEnergyCalculationNetwork, scope: Scope) -> FreeEnergyCalculationNetwork:
        """
    ...
    def action_network(self, planned_network: FreeEnergyCalculationNetwork) -> list[Optional[ScopedKey]]:
        """
        ...
        for tf_sk in self._client.get_network_transformations(network_key):
            ...
            tasks.extend(self._client.create_tasks(tf_sk, count=planned_network.n_repeats + 1))
    ...
    def network_status(self, planned_network: FreeEnergyCalculationNetwork) -> Dict[str, int]:
        """
    ...
    def collect_results(self, planned_network: FreeEnergyCalculationNetwork, allow_missing: bool = False) -> FreeEnergyCalculationNetwork:
        """
        ...
        if not allow_missing and len(status) > 1:
            raise RuntimeError("Not all calculations have finished, to collect the current results use `allow_missing=True`.")

        # collect results following the notebook from openFE
        results = dict()
        for tf_sk in self._client.get_network_transformations(planned_network.results.network_key):
