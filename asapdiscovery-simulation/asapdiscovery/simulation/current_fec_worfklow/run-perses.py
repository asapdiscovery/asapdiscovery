...
def enumerate_transformations():
    ...
    for prefix, transformation_group in setup_options["transformation_groups"].items():
        ...
        for assembly_state, receptor_protonation_state in product(
            setup_options["assembly_states"],
            setup_options["receptor_protonation_states"],
        ):
            ...
            transformation[
                "protein_pdb"
            ] = f"../receptors/{assembly_state}/Mpro-{fragalysis_id}_bound-{receptor_protonation_state}-protein.pdb"
            ...
            for old_ligand_index in range(min(NSTARMAPS, nmolecules)):
                for new_ligand_index in range(old_ligand_index + 1, nmolecules):
                    ...
                    ] = f"{prefix}-{receptor_protonation_state_safe}-{old_ligand_index}-{new_ligand_index}"
...
@cli.command()
@click.option("--index", type=int, required=True, help="Index of calculation to run.")
def run(index):
    # Load options
    import yaml
    ...
    run(
        yaml_filename=setup_options["perses_yaml_template"],
