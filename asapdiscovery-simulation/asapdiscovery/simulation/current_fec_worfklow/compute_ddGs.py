...
def augment_with_expt_from_manual_insert(filename, graph):
    ...
    * `sars mpro pic50`    
    ...
    expt_mols = [ mol for mol in Chem.SDMolSupplier(filename) if "sars mpro pic50" in list(mol.GetPropNames()) ]
    expt_dict = { mol.GetProp("_Name") : mol.GetProp("sars mpro pic50") for mol in expt_mols}
    ...
    for name, pic50 in expt_dict.items():
        ...
        try:
            ...
            if not success:
                print(f"Attempted insert of {name} experimental data ({pic50}) but node not found in graph. Ignoring.")
    ...
    * `r_epik_State_Penalty`
    ...
    with oechem.oemolistream(filename) as ifs:
        ...
        while oechem.OEReadMolecule(ifs, oemol):
            title = titles[molecule_index] # use title with warts added
            try:
                state_penalty_in_kT = float(oechem.OEGetSDData(oemol, 'r_epik_State_Penalty')) * unit.kilocalories_per_mole  / kT # protonation/tautomeric state penalty
            except ValueError:
    ...
    * Enable user to specify one or more experimental measurements that can be passed to DiffNet to improve the resulting estimate
    ...
    for filename in track(yaml_filenames, description='[blue]Retrieving results of perses calculations...'):
        ...
        if not basepath == ".": # need to correct the relative path in the YAML.
            ligand_file = "/".join(basepath.split("/")[:-2])+perses_input['ligand_file'].replace("..","")
        else:
        ...
        statistics = get_perses_realtime_statistics(basepath+path, trajectory_prefix=trajectory_prefix)
        ...
        if ('solvent' in statistics) and ('complex' in statistics):
            ...
            edge_fe = statistics['complex'][-1]['mbar_analysis']['free_energy_in_kT'] - statistics['solvent'][-1]['mbar_analysis']['free_energy_in_kT']
            edge_dfe = np.sqrt(statistics['complex'][-1]['mbar_analysis']['standard_error_in_kT']**2 + statistics['solvent'][-1]['mbar_analysis']['standard_error_in_kT']**2)
            if edge_dfe == 0:
            ...
            graph.nodes[old_ligand_title]['ligand_title'] = ligand_titles[old_ligand_index]
            graph.nodes[new_ligand_title]['ligand_title'] = ligand_titles[new_ligand_index]
...
def gather_additional_data(sorted_ligands, path_to_postera_csv):
    ...
    for ligand_tag in sorted_ligands:
        nowart_ligand_tag = ligand_tag.split("_")[0] # JS -> needed to match with non-warted ASAP MOL NAME
        found = False
        for row in postera_csv:
            if ligand_tag in row[1] or nowart_ligand_tag in row[0] or nowart_ligand_tag in row[1]: # InChIs have been truncated but still specific enough.
                postera_rows.append(row)
                found = True
        if not found: # if ligand is not in input postera data it must be a reference crystal.
...
def write_predictions(original_postera_data_sorted, ligand_dgs_sorted, ligand_uncs_sorted, out_file, headers,
                        ref_lig_name, ref_lig):
    """
    ...
    with open(f"{out_file}.csv", "w") as writefile:
        ...
        for row, dg, unc in zip(original_postera_data_sorted, ligand_dgs_sorted, ligand_uncs_sorted):
            if row[0] == "reference":
...
def write_structures(receptor_path, ref_lig, ref_lig_name, ligands_file, out_file):
    ...
    mol_dict = {}
    for mol in mols: # these will be inchis for prospective and a fragalysis ID for reference.
        mol_dict[mol.GetProp("_Name")] = mol

    # now load the previously written CSV output.
    with open(f"{out_file}.csv", "r") as readfile, \
        Chem.SDWriter(f"{out_file}_ligands.sdf") as output_sdf:
        reader = csv.reader(readfile)
        ...
        for row in reader:
            ...
            if not mol:
                ...
                raise ValueError(f"Unable to match {out_file}.csv entry {row[1]} to docked ligands {ligands_file}.")
...
def check_args(basepath, ligands_file, receptor_file):
    ...
    if not glob.glob(f"{basepath}/*-His163(*)-*-*"):
        raise ValueError(f"No perses simulation folders called *-His163(*)-*-* found in {basepath}.")
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument("--dir", type=str, help="Base directory where results live", required=True)
    arg_parser.add_argument("--docked", type=str, help="Name of the ligands sdf file in the base directory.", required=True)
    arg_parser.add_argument("--receptor", type=str, help="Path to receptor that was used in simulations. Also uses reference ligand.sdf from this path.", required=True)
    arg_parser.add_argument("--input_csv", type=str, help="Name of the postera ligands csv file in the base directory.", required=True)
    arg_parser.add_argument("--out", type=str, help="Name of output files containing data.", required=True)
    ...
    sorted_ligands, ligand_dgs_sorted, ligand_uncs_sorted = display_predictions(collapsed_graph)


    ref_lig_name, ref_lig = get_reference_data(receptor_path, basepath)

    original_postera_data_sorted, headers = gather_additional_data(sorted_ligands, input_csv)

    written_df = write_predictions(original_postera_data_sorted, ligand_dgs_sorted, 
                                    ligand_uncs_sorted, out_file, headers,
                                    ref_lig_name, ref_lig)
