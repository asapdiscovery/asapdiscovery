...
def GetFragments(mol, minbonds, maxbonds):
    ...
    frags = []
    fptype = oegraphsim.OEGetFPType(
        "Tree,ver=2.0.0,size=4096,bonds=%d-%d,atype=AtmNum,btype=Order"
        % (minbonds, maxbonds)
    )
...
def GetCoreFragment(
    ...
    if len(frags) == 0:
        oechem.OEThrow.Error(
            "No fragment is enumerated with bonds %d-%d!" % (minbonds, maxbonds)
        )
...
def expand_stereochemistry_star(mol):
    ...
    forceFlip = False
    enumNitrogen = (
        True  # WARNING: This creates multiple microstates with same SMILES if True
    )
    ...
    for enantiomer in oeomega.OEFlipper(
        mol, maxcenters, forceFlip, enumNitrogen, warts
    ):
...
def expand_stereochemistry(mols):
    ...
    for enantiomers in track(
        pool.imap_unordered(expand_stereochemistry_star, mols),
        total=len(mols),
        description="Enumerating stereocenters...",
    ):
...
def mmff_energy(mol):
    ...
    if not mmff.PrepMol(mol) or not mmff.Setup(mol):
        oechem.OEThrow.Warning(
            "Unable to process molecule: title = '%s'" % mol.GetTitle()
        )
        return None
...
def generate_restricted_conformers(receptor, refmol, mol, core_smarts=None):
    ...
    logging.debug(
        f"mol: {oechem.OEMolToSmiles(mol)} | refmol: {oechem.OEMolToSmiles(refmol)} : core_smarts: {core_smarts}"
    )
    ...
    initial_chiral_centers = {
        oeatom.GetName(): oechem.OEPerceiveCIPStereo(mol, oeatom)
        for oeatom in mol.GetAtoms(oechem.OEIsChiralAtom())
        if oechem.OEGetAtomicSymbol(oeatom.GetAtomicNum()) == "C"
    }
    for name, cip in initial_chiral_centers.items():
        if cip == oechem.OECIPAtomStereo_UnspecStereo:
            print(
                f"Molecule has unspecified stereochemistry: {oechem.OECreateCanSmiString(mol)} at atom {name}: {cip}"
            )
    ...
    if initial_chiral_centers != final_chiral_centers:
        print(
            f"Rejecting due to stereochemistry inversion: {initial_chiral_centers} -> {final_chiral_centers}"
        )
        return None
...
def get_series(mol):
    ...
    try:
        ...
        for warhead in covalent_warheads:
            if (
                oechem.OEHasSDData(mol, warhead)
                and oechem.OEGetSDData(mol, warhead) == "True"
            ):
                return None
    except Exception as e:
        logging.warning(e)
    ...
    def check_if_smi_in_series(
        ...
        if (
            (len(Chem.AddHs(Chem.MolFromSmiles(smi)).GetSubstructMatches(patt)) > 0)
            and (MW <= MW_cutoff)
...
def generate_restricted_conformers_star(args):
    ...
    try:
        ...
        energies = [
            float(oechem.OEGetSDData(mol, "MMFF_internal_energy")) for mol in mols
        ]
...
def generate_restricted_conformers_best(args):
    ...
    # Build with all core_smarts options
    mols = [
        generate_restricted_conformers(receptor, refmol, mol, core_smarts=core_smarts)
        for core_smarts in core_smarts_list
        for mol in mols
    ]
    ...
    try:
        ...
        mols = [
            mol
            for mol in mols
            if float(oechem.OEGetSDData(mol, "MMFF_internal_energy")) < ENERGY_CUTOFF
        ]
...
def generate_poses(
    receptor, refmol, target_molecules, output_filename, expand_protonation_states=False
):
    """
    Parameters
    ----------
    ...
    logging.info(
        f"  There are {len(target_molecules)} target molecules after expanding uncertain stereochemistry"
    )
    ...
    logging.info(
        f"  There are {len(target_molecules)} target molecules after expanding protonation states"
    )
...
def filter_by_series(mols, filter_series="3-aminopyridine-like"):
    ...
    logging.info(
        f"Filtering by series {filter_series} retains {len(filtered_mols)} / {len(mols)} molecules."
    )
...
def filter_by_IC50s(mols):
    ...
    logging.info(
        f"Filtering only molecules with IC50s retains {len(filtered_mols)} / {len(mols)} molecules."
    )
    return filtered_mols
...
def load_fragment(fragid, protonation_state, title=None):
    ...
    refmol_filename = (
        f"./receptors/dimer/Mpro-{fragid}_bound-{protonation_state}-ligand.mol2"
    )
    refmol_filename = f"./receptors/dimer/SARS2_Mpro_ASAP-0000272-prepped_complex-His41(0)-Cys145(0)-His163(0)-ligand.mol2"
    refmol = None
...
def prepend_fragment_molecules(mols, fragments, protonation_states):
    """
    ...
    """
    titles = {mol.GetTitle() for mol in mols}
    ...
    for fragid, cid in fragments.items():
        if cid not in titles:
            # Add the molecule
            if type(protonation_states) == str:
                protonation_state = protonation_states
            else:
                protonation_state = protonation_states[0]

            fragment = load_fragment(
                fragid, title=cid, protonation_state=protonation_state
            )
...
def read_receptor(fragid, assembly_state, protonation_state):
    ...
    receptor_filename = (
        f"./receptors/dimer/Mpro-{fragid}_bound-{protonation_state}-receptor.oeb.gz"
    )
    receptor_filename = f"./receptors/dimer/SARS2_Mpro_ASAP-0000272-prepped_complex-His41(0)-Cys145(0)-His163(0)-receptor.oeb.gz"
    from openeye import oedocking
...
if __name__ == "__main__":
    ...
    fragments = {
        entry["fragalysis_id"]: entry["postera_id"]
        for entry in setup_options["source_structures"]
    }
    ...
    for prefix in molecule_sets_to_dock:
        ...
        logging.info(
            f"Reading molecules to be docked from {target_molecules_filename}..."
        )
        ...
        prepend_fragment_molecules(
            target_molecules, fragments, receptor_protonation_states
        )
            ...
            for assembly_state in ["dimer"]:
                ...
                for receptor_protonation_state in receptor_protonation_states:
                    ...
                    receptor = read_receptor(
                        fragment, assembly_state, receptor_protonation_state
                    )

                    # Read reference fragment with coordinates
                    refmol = load_fragment(
                        fragment, receptor_protonation_state, title=fragments[fragment]
                    )
                    ...
                    generate_poses(
                        ...
                        molecules_to_dock,
                        output_filename,
                        expand_protonation_states=expand_protonation_states,
                    )
