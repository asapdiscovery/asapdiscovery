...
class _NetworkPlannerSettings(_SchemaBase):
    ...
    network_planning_method: Literal[
        "radial", "maximal", "minimal_spanning", "lomap"
    ] = Field(
        "radial",
...
class LocalResults(_SchemaBase):
    type: Literal["LocalResults"] = "LocalResults"


class AlchemiscaleResults(_SchemaBase):
...
class FreeEnergyPerturbationNetwork(_SchemaBase):
    ...
    results: Optional[LocalResults, AlchemiscaleResults] = Field(
        ...,
    ...
    def _get_openfe_protocol(self):
        """Build the corresponding protocol from the settings module"""
    ...
    def to_alchemical_network(self) -> openfe.AlchemicalNetwork:
        ...
        protocol = self._get_openfe_protocol()
