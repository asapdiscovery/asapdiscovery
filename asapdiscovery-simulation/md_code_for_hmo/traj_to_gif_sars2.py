...
if arguments["--pse"]:
    log.info(
        f":page_facing_up:  Writing PyMol ensemble to session_1_loaded_system.pse..."
    )
...
for subpocket_name, residues in pocket_dict.items():
    cmd.select(
        subpocket_name, f"{complex_name} and resi {residues} and polymer.protein"
    )
...
if arguments["--pse"]:
    log.info(
        f":page_facing_up:  Writing PyMol ensemble to session_2_colored_subpockets.pse..."
    )
...
if arguments["--pse"] or arguments["--pse_share"]:
    log.info(
        f":page_facing_up:  Writing PyMol ensemble to session_3_set_ligand_view.pse..."
    )
...
if arguments["--pse"]:
    log.info(
        f":page_facing_up:  Writing PyMol ensemble to session_4_loaded_trajectory.pse..."
    )
...
if arguments["--smooth"]:
    cmd.smooth(
        "all", window=int(arguments["--smooth"])
...
if arguments["--pse"]:
    log.info(
        f":page_facing_up:  Writing PyMol ensemble to session_5_intrafitted.pse..."
    )
