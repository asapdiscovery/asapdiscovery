...
from bokeh.models import Range1d, Band, ColumnDataSource, Whisker
import plotmol
...
def ki_to_dg(
    ...
    if ki > 1e-15 * unit.nanomolar:
        dg = (unit.molar_gas_constant * temperature.to(unit.kelvin)
              * np.log(ki / unit.molar)).to(unit.kilocalorie_per_mole)
    else:
        raise ValueError("negative Ki values are not supported")
    if uncertainty > 0 * unit.molar:
        ddg = abs(unit.molar_gas_constant * temperature.to(unit.kelvin) * np.log(uncertainty / unit.molar)).to(unit.kilocalorie_per_mole)
    else:
...
def shift_and_add_prediction_error(df: pd.DataFrame, point_type: str) -> pd.DataFrame:
    ...
    if point_type == "DG":
        ...
        df[f"{point_type} (kcal/mol) (FECS)"] = df[f"{point_type} (kcal/mol) (FECS)"] - df[f"{point_type} (kcal/mol) (FECS)"].mean() + df[f"{point_type} (kcal/mol) (EXPT)"].mean()

    # calculate the prediction error.
    df["prediction error (kcal/mol)"] = abs(df[f"{point_type} (kcal/mol) (FECS)"] - df[f"{point_type} (kcal/mol) (EXPT)"])
    return df
...
def dg_to_pic50_dataframe(df):
    ...
    for column in df._get_numeric_data().columns:
        ...
        if "DDG (kcal/mol)" in column:
            df = df.rename(columns={column : f"{column.replace('DDG (kcal/mol)', 'relative pIC50')}"})
        elif "DG (kcal/mol)" in column:
            df = df.rename(columns={column : f"{column.replace('DG (kcal/mol)', 'pIC50')}"})
        elif "uncertainty (kcal/mol)" in column:
            df = df.rename(columns={column : f"{column.replace('uncertainty (kcal/mol)', 'pIC50 uncertainty')}"})
        elif "prediction error (kcal/mol)" in column:
            df = df.rename(columns={column : f"{column.replace('prediction error (kcal/mol)', 'prediction error (pIC50)')}"})
...
def add_smiles_to_df(dataframe: pd.DataFrame, ligands: list) -> pd.DataFrame:
    ...
    expt_smiles = dict(
        (ligand.name, ligand.smiles) for ligand in ligands
    )
...
def extract_experimental_data(reference_csv: str, assay_units: Literal['pIC50', 'IC50']) -> dict[str, tuple[unit.Quantity, unit.Quantity]]:
    """
    ...
    if assay_endpoint_tag is None or assay_endpoint_confidence_tag is None:
        raise RuntimeError(f"Could not determine the assay tag from the provided units {assay_units}.")
...
def add_absolute_expt(dataframe: pd.DataFrame, experimental_data: dict[str, tuple[unit.Quantity, unit.Quantity]]):
    """
...
def add_relative_expt(dataframe: pd.DataFrame, experimental_data: dict[str, tuple[unit.Quantity, unit.Quantity]]):
    """
    ...
    for _, row in dataframe.iterrows():
        ...
        ddg = get_magnitude(experimental_data[label_b][0]) - get_magnitude(experimental_data[label_a][0])
        # take the average uncertainty between measurements for this edge.
        delta_ddg = np.mean([get_magnitude(experimental_data[label_a][1]), get_magnitude(experimental_data[label_b][1])])
...
def get_data_from_femap(fe_map: cinnabar.FEMap, ligands: list, assay_units: Optional[str] = None, reference_dataset: Optional[str] = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    if reference_dataset:
        experimental_data = extract_experimental_data(reference_csv=reference_dataset, assay_units=assay_units)
...
def _plot_with_plotmol(
    ...
    figure.line([ax_min, ax_max], [ax_min, ax_max], line_color="black", line_dash='dashed')
    ...
    in_band = Band(base="x", upper="upper_in", lower="lower_in", source=band_data, fill_alpha=0.2, fill_color="grey")
    outer_band = Band(base="x", upper="upper_out", lower="lower_out", source=band_data, fill_alpha=0.2, fill_color="grey")
...
def create_absolute_report(dataframe: pd.DataFrame) -> panel.Column:
    ...
    for statistic in ["RMSE", "MUE", "R2", "rho"]:
        s = stats.bootstrap_statistic(dataframe["DG (kcal/mol) (EXPT)"],
        ...
        stats_data.append(
            {"Statistic": statistic, "value": s["mle"], "lower bound": s["low"], "upper bound": s["high"]})
    ...
    layout = panel.Column(
        ...
        panel.widgets.Tabulator(
            ...
            selectable="checkbox",
            disabled=True,
            formatters={"SMILES": "html", "Molecule": "html", "DG (kcal/mol) (FECS)": number_format,
                        "uncertainty (kcal/mol) (FECS)": number_format, "DG (kcal/mol) (EXPT)": number_format,
                        "uncertainty (kcal/mol) (EXPT)": number_format, "prediction error (kcal/mol)": number_format},
...
def create_relative_report(dataframe: pd.DataFrame) -> panel.Column:
    ...
    stats_data = []
    for statistic in ["RMSE", "MUE", "R2", "rho"]:
        s = stats.bootstrap_statistic(dataframe["DDG (kcal/mol) (EXPT)"],
    ...
    layout = panel.Column(
        ...
        panel.widgets.Tabulator(
            ...
            selectable="checkbox",
            disabled=True,
            formatters={"SMILES_A": "html", "SMILES_B":"html", "Molecules": "html", "DDG (kcal/mol) (FECS)": number_format,
                        "uncertainty (kcal/mol) (FECS)": number_format, "DDG (kcal/mol) (EXPT)": number_format,
                        "uncertainty (kcal/mol) (EXPT)": number_format, "prediction error (kcal/mol)": number_format},
            configuration={"rowHeight": 300},
        ),
        sizing_mode="stretch_width",
