...
"""
import json
from asapdiscovery.alchemy.schema.fec import FreeEnergyCalculationNetwork, AlchemiscaleResults, TransformationResult
...
def main():
    ...
    new_schema = FreeEnergyCalculationNetwork(network=old_schema["network"], receptor=old_schema["receptor"], dataset_name=old_schema["dataset_name"])
    ...
    for result in old_schema["results"]["results"]:
        new_result = TransformationResult(
            ...
            phase=result["phase"],
            estimate=result["estimate"]["magnitude"] * getattr(unit, result["estimate"]["unit"]),
            uncertainty=result["uncertainty"]["magnitude"] * getattr(unit, result["uncertainty"]["unit"])
        )
        results.append(new_result)
    all_results = AlchemiscaleResults(network_key=old_schema["results"]["network_key"], results=results)
    new_schema = FreeEnergyCalculationNetwork(network=old_schema["network"], receptor=old_schema["receptor"], dataset_name=old_schema["dataset_name"], results=all_results)
