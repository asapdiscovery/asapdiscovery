...
@pytest.mark.parametrize("strict_stereo, core_smarts, failed", [
    pytest.param(True, None,  True, id="Strict-no-smarts"),
    pytest.param(True, "C(Nc1ncnc2c1cc[nH]2)c3cc(S(=O)(CCC4)=O)c4cc3", False, id="Strict-smarts"),
    pytest.param(False, None, False, id="Lax-stereo"),
    pytest.param(False, "CC(Nc1ncnc2c1cc[nH]2)c3cc(S(=O)(CCC4)=O)c4cc3", False, id="Lax-stereo-smarts")
])
def test_prep_workflow(strict_stereo, core_smarts, failed, mac1_complex):
    ...
    workflow = AlchemyPrepWorkflow(charge_expander=None, strict_stereo=strict_stereo, core_smarts=core_smarts)

    alchemy_dataset = workflow.create_alchemy_dataset(
        dataset_name="mac1-testing-dataset",
        ligands=[Ligand.from_smiles("Cc1c(cn(n1)C)c2cc3c([nH]2)ncnc3N[C@@H](c4ccc5c(c4)S(=O)(=O)CCC5)C(C)C", compound_name="stereo_mol")],
        reference_complex=mac1_complex
    )

    assert len(alchemy_dataset.input_ligands) == 1
    assert alchemy_dataset.provenance == {
        'OpenEyeConstrainedPoseGenerator': {'oechem': 20230910,
                                     'oedocking': 20230910,
                                     'oeff': 20230910,
                                     'oeomega': 20230910},
        'StereoExpander': {'expander': {'expander_type': 'StereoExpander',
    ...
    if failed:
        ...
        assert alchemy_dataset.failed_ligands["InconsistentStereo"][0].compound_name == "stereo_mol"
    else:
