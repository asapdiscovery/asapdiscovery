...
def submit(
    ...
    bespoke_factory.initial_force_field = (
        fec_network.forcefield_settings.small_molecule_forcefield
    )
...
def gather(network: str, allow_missing: bool):
    ...
    for ligand in track(
        fec_network.network.ligands,
        description="Gathering bespoke parameters",
        console=console,
        transient=True,
        total=len(fec_network.network.ligands),
    ):
        ...
        if "bespokefit_id" in ligand.tags:
            ...
            if bespoke_result.status == "success":
                ...
                bespoke_parameters = BespokeParameters(
                    base_force_field=fec_network.forcefield_settings.small_molecule_forcefield
                )
