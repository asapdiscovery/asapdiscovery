...
def molecule_set(args):
    ...
    if args.action == "read":
        ...
        if args.smiles_field not in df.columns:
            raise ValueError(f"Smiles field {args.smiles_field} not found in input file")
        ...
        if args.postera_id_field not in df.columns:
            raise ValueError(f"PostEra molecule id field {args.postera_id_field} not found in input file")
        ...
        molecules_updated = molecule_set.update_custom_data(args.molecule_set_id, update_molecule_list)
