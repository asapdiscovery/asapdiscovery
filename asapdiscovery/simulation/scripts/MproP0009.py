...
# Solvate
modeller.addSolvent(
    system_generator.forcefield, padding=0.9 * unit.nanometers, model="tip3p"
)
...
print("Simulating...")
dcdReporter = simulation.reporters.append(
    DCDReporter(os.path.join(args.output_dir, "trajectory.dcd"), 50)
)
...
xtcReporter = simulation.reporters.append(
    XTCReporter(os.path.join(args.output_dir, "trajectory.xtc"), 50)
)
...
PDBFile.writeFile(
    simulation.topology,
    positions,
    open(os.path.join(args.output_dir, "output_P0009_final.pdb"), "w"),
)
