...
dcdReporter = simulation.reporters.append(
    DCDReporter(os.path.join(args.output_dir, save + ".pdb"), reportingInterval)
)
dataReporter = simulation.reporters.append(
    StateDataReporter(
        os.path.join(args.output_dir, save + "_" + "log.txt"),
        50,
        totalSteps=steps,
        step=True,
        speed=True,
...
xtcReporter = simulation.reporters.append(
    XTCReporter(os.path.join(args.output_dir, save + ".xtc"), reportingInterval)
)
...
PDBFile.writeFile(
    simulation.topology,
    positions,
    open(os.path.join(args.output_dir, save + "_final.pdb"), "w"),
)
