...
def simulate_mpro(input_dir,protein,ligand,nsteps,output_dir):
    ...
    for dim in range(3):
        logger.info(f'  :small_blue_diamond: {system.getDefaultPeriodicBoxVectors()[dim]}')
    ...
    dataReporter = simulation.reporters.append(StateDataReporter(os.path.join(output_dir + save + '_simulation_' +'log.txt'), 50, totalSteps=steps,
        step=True, speed=True, progress=True, potentialEnergy=True, temperature=True, separator='\t'))
    xtcReporter = simulation.reporters.append(XTCReporter(os.path.join(output_dir + save + '_simulation_' +'.xtc'), reportingInterval))
    ...
    PDBFile.writeFile(simulation.topology, positions, open(os.path.join(output_dir + save + '_simulation' + '_final.pdb'), 'w'))
