...
def get_args():
    ...
    parser.add_argument('-labs', nargs='+',
        help='Plot labels (must align with args to -i).')
...
def main():
    ...
    correct_dict, incorrect_dict = zip(
        *[pkl.load(open(fn, 'rb')) for fn in args.i])
    ...
    ## Plotting
    membership_dict = {p: [l for l in labs if p in correct_dict[l]] \
        for p in all_pairs}
    fig = plt.figure(figsize=(12,8))
    ups.UpSet(ups.from_memberships(membership_dict.values()),
        subset_size='count').plot(fig)
    fig.savefig(f'{args.o}.png', dpi=200, bbox_inches='tight')

    ## Formatting to CSV
    membership_arr = np.vstack([[p in correct_dict[l] for l in labs] \
        for p in all_pairs])
