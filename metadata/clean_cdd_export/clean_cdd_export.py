...
cleaned_df = cleaned_df[cleaned_df['suspected_SMILES'].apply(lambda x: True if len(x.split()) == 1 else False)]
...
for idx, row in bulky_df[bulky_df["ProteaseAssay_Fluorescence_Dose-Response_Weizmann: IC50 CI (Lower) (µM)"].isna()].iterrows():
    # set this row's artificial values.
    bulky_df.loc[idx]["ProteaseAssay_Fluorescence_Dose-Response_Weizmann: IC50 (µM)"] = 100
...
for df, output_path in zip([cleaned_df, bulky_df], [clean_output_csv_filename, bulky_output_csv_filename]):
    # Rename
