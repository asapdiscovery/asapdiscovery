...
def main():
    ...
    ## Write out CSV Files
    dr.df.to_csv(os.path.join(args.output_dir, "all_results_cleaned.csv"), index=False)
    dr.compound_df.to_csv(os.path.join(args.output_dir, "by_compound.csv"), index=False)
    dr.structure_df.to_csv(os.path.join(args.output_dir, "by_structure.csv"), index=False)
    dr.best_df.to_csv(os.path.join(args.output_dir, "mers_fauxalysis.csv"), index=False)
