...
def main():
    ...
    sns.scatterplot(x='target', y='pred', hue='train_test', data=compare_df,
        alpha=0.7, ax=ax)
    sns.lineplot(x=[min(target_vals), max(target_vals)],
        y=[min(target_vals), max(target_vals)], ls='--', color='black', ax=ax)
