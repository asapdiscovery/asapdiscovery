...
def load_affinities(fn):
    ...
    for i in range(0, len(exp_compounds), 2):
        p = tuple(sorted((exp_compounds[i].compound_id,
            exp_compounds[i+1].compound_id), key=affinity_dict.get,
            reverse=True))
...
def load_affinities_ep(fn):
    ...
    enant_pairs = [(ep.active.compound_id, ep.inactive.compound_id) \
        for ep in ep_list]
...
def main():
    ...
    print(f'{len(incorrect_pairs)}/{len(enant_pairs)} ranked incorrectly.',
        flush=True)
