...
def get_args():
    ...
    ## Input arguments
    parser.add_argument('-d', "--fauxalysis_dir", required=True,
                        help='Path to fauxalysis_dir.')
    parser.add_argument('-y', "--yaml_file", default="../data/luttens2022ultralarge.yaml",
...
def main():
    ...
    complex_list = [directory for directory in os.listdir(args.fauxalysis_dir) if not ".csv" in directory]

    for pdb in pdb_dict.keys():
        complexes = [complex_id for complex_id in complex_list if pdb.lower() in complex_id]
