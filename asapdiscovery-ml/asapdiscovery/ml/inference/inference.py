...
from asapdiscovery.ml.weights import fetch_weights_from_spec, fetch_weights, all_models_spec
...
class InferenceBase:
    ...
    def __init__(self, model_name: str, model_type: str, from_spec: bool=True, build_model_kwargs: Optional[Dict]=None, device: str='cpu'):
        logging.info(f"initializing {.__class__} class")
        ...
        if from_spec:
            logging.info("using weights from asapdiscovery.ml spec file")
            self.weights =  fetch_weights_from_spec(all_models_spec, model_name)
        else:
...
class GATInference(InferenceBase):
    """
    Inference class for GAT models in ASAP Discovery.

    """
    model_type = "GAT"

    def __init__(self, model_name: str, device:str ='cpu'):
        # build model kwargs specific to the GAT model
        build_model_kwargs = {}
        super().__init__(model_name, self.model_type, build_model_kwargs=build_model_kwargs, device)
