...
def fetch_weights_from_spec(yamlfile: str, models: Union[List[str], str],  local_path: str = "./_weights/", force_fetch: bool = False):
    ...
    for model in models:
        ...
        if weights in filename_set:
            raise ValueError(f"Duplicate file {weights} in spec file. Please specify a unique filename for each model.")
    ...
    if is_url:
        ...
        if force_fetch:
            ...
            if Path(path + local_filename).exists():
                ...
                logging.info(f"weights exist locally at {path + local_filename}, skipping fetch from remote")
        ...
        if not Path(path + weights).exists():
            raise FileNotFoundError(f"Local weights file {path + weights} does not exist, provide a valid remote url or local path to weights")

        else:
            logging.info(f"weights exist locally at {path + weights}, skipping fetch from remote")
