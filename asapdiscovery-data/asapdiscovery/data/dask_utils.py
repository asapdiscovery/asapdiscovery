...
_LILAC_GPU_EXTRAS = {
    GPU.GTX1080TI: ['-R "select[hname!=lt16]"'], # random node that has something wrong with its GPU
}
...
class DaskCluster(BaseModel):
    ...
    death_timeout: int = Field(120, description="Timeout for a worker to be considered dead")
...
class LilacDaskCluster(DaskCluster):
    ...
    job_extra_directives: Optional[list[str]] = Field(None, description="Extra directives to pass to LSF")
...
class LilacGPUConfig(BaseModel):
    ...
    extra: Optional[list[str]] = Field(None, description="Extra directives to pass to LSF")

    def to_job_extra_directives(self):
        return ['-gpu num=1:j_exclusive=yes:mode=shared', f'-m {self.gpu_group}', *self.extra]
    
    @classmethod
    def from_gpu(cls, gpu: GPU):
        return cls(gpu=gpu, gpu_group=_LILAC_GPU_GROUPS[gpu], extra=_LILAC_GPU_EXTRAS[gpu])
