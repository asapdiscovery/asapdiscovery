...
from asapdiscovery.data.postera.molecule_set import MoleculeSetAPI, MoleculeList, MoleculeUpdateList
...
pytestmark = pytest.mark.skipif(not os.getenv("POSTERA_API_KEY"), reason="No POSTERA_API_KEY")

# we have a second environment variable to stop you accidentally running this test with a production API key
# DO NOT REMOVE 
pytestmark_sandbox = pytest.mark.skipif(not os.getenv("POSTERA_API_KEY_IS_SANDBOX"), reason="No POSTERA_API_KEY_IS_SANDBOX")
...
class TestPosteraLive:
    ...
    def simple_moleculeset_molecule_list(self, simple_moleculeset_data):
        return MoleculeList.from_pandas_df(simple_moleculeset_data, smiles_field="smiles_field", id_field="id_field")
    ...
    def test_simple_moleculeset_create(self, live_postera_ms_api_instance, simple_moleculeset_molecule_list):
    ...
    @pytest.fixture()
    def simple_moleculeset(self, live_postera_ms_api_instance, simple_moleculeset_molecule_list):
    ...
    def test_update_set_equal(self, live_postera_ms_api_instance, simple_moleculeset):
        ...
        mlu = MoleculeUpdateList.from_pandas_df(molecules_df, id_field="id", smiles_field="smiles")
        ms_api.update_molecules(uuid, mlu)
        updated_molecules = ms_api.get_molecules(uuid, return_as="dataframe")
        assert updated_molecules["custom_fieldA"].tolist() == ["NEW_CUSTOMA1", "NEW_CUSTOMA2"]

    def test_update_set_strict_subset(self, live_postera_ms_api_instance, simple_moleculeset):
        ...
        assert updated_molecules["custom_fieldA"].tolist() == ["NEW_CUSTOMA3", "CUSTOMA_2"]



    def test_update_set_strict_subset_overwrite(self, live_postera_ms_api_instance, simple_moleculeset):
        # update one subset of molecules
        molecule_set_name, uuid = simple_moleculeset
        ms_api = live_postera_ms_api_instance
        # id and smiles fields will be set to their postera values on pulldown
        ...
        assert updated_molecules["custom_fieldB"].tolist() == [np.nan, 2.0]
