...
class OpenEyeConstrainedPoseGenerator(_BasicConstrainedPoseGenerator):
    ...
    def _generate_poses(
        ...
        self._prune_clashes(receptor=oe_receptor, ligands=result_ligands, near_nbr=near_nbr)
        # select the best pose to be kept
        scorer = self._build_scorer(oedu_receptor)
        posed_ligands = self._select_best_pose(
            receptor=oedu_receptor, ligands=result_ligands, score=scorer
        )
